    ...
    <div class="bg-gray-800 rounded-lg p-6 w-full max-w-2xl my-auto" @click.stop>
      ...
      <form @submit.prevent="handleSubmit">
        <div class="space-y-4">
          ...
          <!-- Special Row Info -->
          <div v-if="row?.is_special" class="bg-yellow-900/20 border border-yellow-700 rounded-md p-3">
...
import { recommendationRows as rowApi, libraries as libraryApi } from '@/api/client'
...
watch(() => form.value.filter_criteria, () => {
  filterCriteriaError.value = ''
}, { deep: true })
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.row) {
      ...
      if (props.libraryId) {
        await rowApi.addLibraryRow(props.libraryId, data)
      } else {
