    ...
    >
      ...
      <div class="progress-container px-4 mb-2">
        ...
        >
          ...
          <div
            v-if="hoverTime !== null"
            class="hover-time absolute -top-8 text-white text-xs bg-black/80 px-2 py-1 rounded"
  ...
  if (audioTracks && audioTracks.length > 0) {
    ...
    if (track.language) {
      for (let i = 0; i < audioTracks.length; i++) {
        ...
        if (audioTrack.language === track.language.toLowerCase() || 
...
function selectSubtitleTrack(track) {
  ...
  if (textTracks && textTracks.length > 0) {
    ...
    if (track) {
      ...
      if (track.language) {
        for (let i = 0; i < textTracks.length; i++) {
          if (textTracks[i].language === track.language.toLowerCase() || 
